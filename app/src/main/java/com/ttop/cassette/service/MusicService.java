...
import com.ttop.cassette.auto.AutoMediaIDHelper;
import com.ttop.cassette.auto.AutoMusicProvider;
import com.ttop.cassette.discog.Discography;
import com.ttop.cassette.discog.tagging.MultiValuesTagUtil;
import com.ttop.cassette.glide.BlurTransformation;
import com.ttop.cassette.glide.GlideApp;
import com.ttop.cassette.glide.GlideRequest;
...
import com.ttop.cassette.helper.ShuffleHelper;
import com.ttop.cassette.model.Playlist;
import com.ttop.cassette.model.Song;
import com.ttop.cassette.provider.HistoryStore;
import com.ttop.cassette.provider.MusicPlaybackQueueStore;
import com.ttop.cassette.provider.SongPlayCountStore;
import com.ttop.cassette.service.notification.PlayingNotification;
import com.ttop.cassette.service.notification.PlayingNotificationImpl;
import com.ttop.cassette.service.notification.PlayingNotificationImpl24;
import com.ttop.cassette.service.playback.Playback;
import com.ttop.cassette.ui.activities.MainActivity;
import com.ttop.cassette.util.MusicUtil;
import com.ttop.cassette.util.PackageValidator;
...
import com.ttop.cassette.volume.AudioVolumeObserver;
import com.ttop.cassette.volume.OnAudioVolumeChangedListener;
 ...
 */
public class MusicService extends MediaBrowserServiceCompat implements SharedPreferences.OnSharedPreferenceChangeListener, Playback.PlaybackCallbacks, OnAudioVolumeChangedListener {

    public static final String TAG = MusicService.class.getSimpleName();

    public static final String CASSETTE_PACKAGE_NAME = "com.ttop.cassette";
    public static final String MUSIC_PACKAGE_NAME = "com.android.music";

    public static final String ACTION_TOGGLE_PAUSE = CASSETTE_PACKAGE_NAME + ".togglepause";
    public static final String ACTION_PLAY = CASSETTE_PACKAGE_NAME + ".play";
    public static final String ACTION_PLAY_PLAYLIST = CASSETTE_PACKAGE_NAME + ".play.playlist";
    public static final String ACTION_PAUSE = CASSETTE_PACKAGE_NAME + ".pause";
    public static final String ACTION_STOP = CASSETTE_PACKAGE_NAME + ".stop";
    public static final String ACTION_SKIP = CASSETTE_PACKAGE_NAME + ".skip";
    public static final String ACTION_REWIND = CASSETTE_PACKAGE_NAME + ".rewind";
    public static final String ACTION_QUIT = CASSETTE_PACKAGE_NAME + ".quitservice";
    public static final String ACTION_PENDING_QUIT = CASSETTE_PACKAGE_NAME + ".pendingquitservice";
    public static final String INTENT_EXTRA_PLAYLIST = CASSETTE_PACKAGE_NAME + "intentextra.playlist";
    public static final String INTENT_EXTRA_SHUFFLE_MODE = CASSETTE_PACKAGE_NAME + ".intentextra.shufflemode";

    public static final String APP_WIDGET_UPDATE = CASSETTE_PACKAGE_NAME + ".appwidgetupdate";
    public static final String EXTRA_APP_WIDGET_NAME = CASSETTE_PACKAGE_NAME + "app_widget_name";

    // Do not change these three strings as it will break support with other apps (e.g. last.fm scrobbling)
    public static final String META_CHANGED = CASSETTE_PACKAGE_NAME + ".metachanged";
    public static final String QUEUE_CHANGED = CASSETTE_PACKAGE_NAME + ".queuechanged";
    public static final String PLAY_STATE_CHANGED = CASSETTE_PACKAGE_NAME + ".playstatechanged";

    public static final String FAVORITE_STATE_CHANGED = CASSETTE_PACKAGE_NAME + "favoritestatechanged";

    public static final String REPEAT_MODE_CHANGED = CASSETTE_PACKAGE_NAME + ".repeatmodechanged";
    public static final String SHUFFLE_MODE_CHANGED = CASSETTE_PACKAGE_NAME + ".shufflemodechanged";
    public static final String MEDIA_STORE_CHANGED = CASSETTE_PACKAGE_NAME + ".mediastorechanged";

    public static final String CYCLE_REPEAT = CASSETTE_PACKAGE_NAME + ".cyclerepeat";
    public static final String TOGGLE_SHUFFLE = CASSETTE_PACKAGE_NAME + ".toggleshuffle";
    public static final String TOGGLE_FAVORITE = CASSETTE_PACKAGE_NAME + ".togglefavorite";
    ...
    private final AppWidgetFull appWidgetFull = AppWidgetFull.getInstance();
    private final AppWidgetMini appWidgetMini = AppWidgetMini.getInstance();
                ...
                public void onReceive(final Context context, final Intent intent) {
                    ...
                    if (action != null) {
                        if (BluetoothDevice.ACTION_ACL_CONNECTED.equals(action)
                                && PreferenceUtil.getInstance().getHeadsetPlayback()) {
                            if (getAudioManager().getDevices(AudioManager.GET_DEVICES_OUTPUTS).length > 0) {
                                if (!MusicPlayerRemote.getPlayingQueue().isEmpty()) {
                                    new Handler().postDelayed(MusicPlayerRemote::resumePlaying, 2000);
                                }
                            }
                        }
                    }
    ...
    public void onCreate() {
        ...
        sendBroadcast(new Intent(CASSETTE_PACKAGE_NAME + ".VINYL_MUSIC_PLAYER_MUSIC_SERVICE_CREATED"));

        mediaStoreObserver.onChange(true);

        if (audioVolumeObserver == null) {
            audioVolumeObserver = new AudioVolumeObserver(getApplicationContext());
        }
    ...
    private void registerHeadsetEvents() {
        if (!headsetReceiverRegistered && PreferenceUtil.getInstance().getHeadsetPlayback()) {
    ...
    private void setupMediaSession() {
        ...
        mediaSession = new MediaSessionCompat(this, "Cassette", mediaButtonReceiverComponentName, mediaButtonReceiverPendingIntent);
                ...
                switch (action) {
                    case ACTION_TOGGLE_PAUSE:
                        if (!playingQueue.isEmpty()) {
                            ...
                            if (PreferenceUtil.getInstance().getEmptyQueueAction()) {
    ...
    public void onDestroy() {
        ...
        sendBroadcast(new Intent(CASSETTE_PACKAGE_NAME + ".CASSETTE_MUSIC_SERVICE_DESTROYED"));
    }
    ...
    private void updateMediaSessionMetaData() {
        ...
        if (PreferenceUtil.getInstance().albumArtOnLockscreen()) {
            ...
            runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    request.into(new CassetteSimpleTarget<Bitmap>(screenSize.x, screenSize.y) {
                        @Override
    ...
    public void playSongAtImpl(int position, boolean play) {
        if (openTrackAndPrepareNextAt(position)) {
            if (play) {
                play();
            }
    ...
    public void sendPublicIntent(@NonNull final String what) {
        final Intent intent = new Intent(what.replace(CASSETTE_PACKAGE_NAME, MUSIC_PACKAGE_NAME));
        ...
        intent.putExtra("scrobbling_source", CASSETTE_PACKAGE_NAME);
    ...
    private void sendChangeInternal(final String what) {
        ...
        appWidgetMini.notifyChange(this, what);
    }
    ...
    public void onAudioVolumeChanged(int currentVolume, int maxVolume) {
        if (currentVolume < 1) {
            setPauseWhenZeroVolume(true);
        }
    }

    private void setPauseWhenZeroVolume(Boolean pauseWhenZeroVolume) {
        if (PreferenceUtil.getInstance().getPauseZero()) {
    ...
    private final BroadcastReceiver widgetIntentReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(final Context context, final Intent intent) {
            ...
            switch (command) {
                ...
                case AppWidgetFull.NAME: {
                    appWidgetFull.performUpdate(MusicService.this, ids);
                    break;
                }
                case AppWidgetMini.NAME: {
                    appWidgetMini.performUpdate(MusicService.this, ids);
                    break;
                }
            }
        }

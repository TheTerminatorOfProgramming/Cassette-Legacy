...
import com.ttop.cassette.adapter.album.HorizontalAlbumAdapter;
import com.ttop.cassette.adapter.song.ArtistSongAdapter;
import com.ttop.cassette.databinding.ActivityArtistDetailBinding;
...
import com.ttop.cassette.lastfm.rest.model.LastFmArtist;
...
import com.ttop.cassette.util.CustomArtistImageUtil;
...
public class ArtistDetailActivity extends AbsSlidingMusicPanelActivity implements PaletteColorHolder, CabHolder, LoaderManager.LoaderCallbacks<Artist> {
    ...
    private void loadArtistImage() {
        GlideApp.with(this)
                .asBitmapPalette()
                .load(CassetteGlideExtension.getArtistModel(artist, forceDownload))
                .transition(CassetteGlideExtension.getDefaultTransition())
                .artistOptions(artist)
                .dontAnimate()
                .into(new CassetteColoredTarget(artistImage) {
                    @Override

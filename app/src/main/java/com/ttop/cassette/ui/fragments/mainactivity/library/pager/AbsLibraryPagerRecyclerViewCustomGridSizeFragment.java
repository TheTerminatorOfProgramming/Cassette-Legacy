...
public abstract class AbsLibraryPagerRecyclerViewCustomGridSizeFragment<A extends RecyclerView.Adapter, LM extends RecyclerView.LayoutManager> extends AbsLibraryPagerRecyclerViewFragment<A, LM> {
    ...
    public final int getGridSize() {
        if (gridSize == 0) {
            if (Util.isTablet(getContext())){
                if (isLandscape()) {
                    gridSize = loadGridSizeTabletLand();
                } else {
                    gridSize = loadGridSizeTablet();
                }
            }else{
                if (isLandscape()) {
    ...
    protected abstract int loadGridSizeTablet();

    protected abstract void saveGridSizeTablet(int gridColumns);

    protected abstract int loadGridSizeTabletLand();

    protected abstract void saveGridSizeTabletLand(int gridColumns);
